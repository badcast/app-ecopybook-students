...
namespace Farbox.Packer
{
    ...
    public class PackFile : IPacker, IDisposable
    {
        ...
        public bool AutoDestruct()
        {
            try
            {
                ...
                return ElectronicLib.G_G.destroy(ref buff);
            }
            finally
            {
    ...
    public class PackInfo : IPacker
    {
        ...
        {
            string extractName = Path.GetFileName(pPath);
        ...
        {
            string extractName = Path.GetFileName(pPath);
            string extractDirectory = Path.GetDirectoryName(pPath);
        ...
        public void Compact(string directoryPath)
        {
            ...
            foreach (var f in filesA)
            {
                ...
                string dir = Path.GetDirectoryName(changed);
                ...
                dr.AddFile(new PackFile(this.packer, File.ReadAllBytes(f), Path.GetFileName(f)));
            }
        }
        ...
        private object GetOfPath(string pPath, FindType findType)
        {
            ...
            for (int i = 0; i < split.Length; i++)
            {
                ...
                for (int l = 0; l < findDirs.Length; l++)
                {
                    ...
                    {
                        PackDirectory d = GetDirectoryFromPath(Path.GetDirectoryName(pPath));
                        if (d == null)
                            return null;

                        item = d;
                        level = d.Name.ToLower();
                        string fileName = Path.GetFileName(pPath).ToLower();
